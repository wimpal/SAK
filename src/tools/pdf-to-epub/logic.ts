...
export function defaultEpubStem(sourcePath: string): string {
  return fileStem(sourcePath);
}

export function epubOutputPath(
  sourcePath: string,
  outputDir: string | null,
  stem: string,
): string {
  const dir = outputDir ?? dirname(sourcePath);
  return joinPath(dir, `${sanitizeOutputStem(stem)}.epub`);
}
...
export function makeJobId(): string {
  return `pdf-to-epub-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
}
