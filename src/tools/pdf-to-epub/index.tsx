...
export default function PdfToEpubTool() {
  ...
  const selectPdf = useCallback((path: string) => {
    ...
    const stem = sanitizeOutputStem(defaultEpubStem(path));
    setSourcePath(path);
    setOutputDir(null);
    setOutputStem(stem);
    setBookTitle(stem);
    ...
    void listen<EbookConvertProgressPayload>(
      ...
      (event) => {
        ...
        if (event.payload.ratio > 0) {
          setProgressLabel(`Converting… ${Math.round(event.payload.ratio * 100)}%`);
        }
      },
    ).then((fn) => {
      unlisten = fn;
  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (result.cleanup?.action === "cleaned" && result.cleanup.removedSpineItems > 0) {
        setSummary(
          `Created ${basename(result.path)} (removed ${result.cleanup.removedSpineItems} duplicate TOC entries)`,
        );
      } else if (result.cleanup?.action === "skipped" && result.cleanup.skippedReason) {
        setSummary(
          `Created ${basename(result.path)} (structure cleanup skipped: ${result.cleanup.skippedReason})`,
        );
      } else {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-zinc-400">
        Convert a PDF into a reflowable EPUB using the bundled Calibre engine.
        Layout and typography may differ from the original PDF — this works best
        for text-heavy documents. Scanned PDFs and password-protected files are
        not supported.
      </p>
      ...
      <section className="space-y-3">
        ...
        {sourcePath && (
          <p className="text-sm text-zinc-300 font-mono truncate" title={sourcePath}>
            {basename(sourcePath)}
          </p>
        )}
      </section>
      ...
      {sourcePath && (
        <section className="space-y-3">
          ...
          {resolvedOutput && (
            <p className="text-xs text-zinc-500 font-mono truncate" title={resolvedOutput}>
