...
const STORAGE = {
  audioCodec: "sak.audio-convert.audioCodec",
  bitrate: "sak.audio-convert.bitrate",
  recursive: "sak.audio-convert.recursive",
  outputDir: "sak.audio-convert.outputDir",
  namePattern: "sak.audio-convert.namePattern",
} as const;
...
function makeJobId(): string {
  return `audio-convert-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
}

export default function AudioConvertTool() {
  const [paths, setPaths] = useState<string[]>([]);
  const [audioCodec, setAudioCodec] = useState<AudioCodec>(() => {
    const stored = loadString(STORAGE.audioCodec, "aac");
    return isAudioCodec(stored) ? stored : "aac";
  });
  const [bitrateKbps, setBitrateKbps] = useState(() => {
    const codecStored = loadString(STORAGE.audioCodec, "aac");
    const codec = isAudioCodec(codecStored) ? codecStored : "aac";
    const stored = Number(loadString(STORAGE.bitrate, String(defaultBitrateFor(codec))));
    return clampBitrate(codec, stored);
  });
  ...
  useEffect(() => {
    saveString(STORAGE.audioCodec, audioCodec);
  }, [audioCodec]);

  useEffect(() => {
    saveString(STORAGE.bitrate, String(bitrateKbps));
  }, [bitrateKbps]);
  ...
  useEffect(() => {
    setBitrateKbps((prev) => clampBitrate(audioCodec, prev));
  }, [audioCodec]);

  // When the batch is all WebM (or a single WebM), auto-switch to Opus.
  useEffect(() => {
    if (paths.length === 0) return;
    const containers = paths
      .map((path) => containerFromPath(path))
      .filter((c): c is NonNullable<typeof c> => c !== null);
    if (containers.length === 0) return;
    const allWebm = containers.every((c) => c === "webm");
    if (allWebm && audioCodec !== "opus") {
      setAudioCodec("opus");
      setBitrateKbps(defaultBitrateFor("opus"));
    }
  }, [paths, audioCodec]);
  ...
  useEffect(() => {
    if (paths.length === 0) {
      ...
      const codec = defaultCodecForExt(fileExtension(paths[0]));
      setAudioCodec(codec);
      setBitrateKbps(defaultBitrateFor(codec));
    }
  }, [paths]);
  ...
  async function convertAll() {
    ...
    for (let i = 0; i < ready.length; i++) {
      ...
      results.set(row.from, { status: "converting" });
      setRowResults(new Map(results));
      ...
      try {
        const result = await invoke<ConvertResult>("convert_audio", {
          jobId,
          sourcePath: row.from,
          outputPath: row.to,
          audioCodec,
    ...
    const parts = [`${succeeded} converted`];
  ...
  const bitrateOptions = bitrateOptionsFor(audioCodec);
  const hasWebm = paths.some((path) => containerFromPath(path) === "webm");
  const hasNonWebm = paths.some((path) => {
    const c = containerFromPath(path);
    return c !== null && c !== "webm";
  });
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="text-sm text-zinc-400">Audio codec</span>
          <select
            value={audioCodec}
            onChange={(e) => {
              const value = e.target.value;
              if (!isAudioCodec(value)) return;
              setAudioCodec(value);
              setBitrateKbps(defaultBitrateFor(value));
              setSummary(null);
              setLastSavedPath(null);
              setRowResults(new Map());
            }}
          ...
          >
            <option value="aac" disabled={hasWebm && !hasNonWebm}>
              AAC
            </option>
            <option value="opus">Opus</option>
          </select>
          <p className="mt-1 text-xs text-zinc-600">
            WebM requires Opus. Other containers default to AAC.
          </p>
        </label>
        ...
        <label className="block">
          ...
          <select
            value={bitrateKbps}
            onChange={(e) => {
              setBitrateKbps(clampBitrate(audioCodec, Number(e.target.value)));
              setSummary(null);
              setLastSavedPath(null);
              setRowResults(new Map());
            }}
          ...
          >
            {bitrateOptions.map((kbps) => (
              <option key={kbps} value={kbps}>
                {kbps} kbps
                {kbps === defaultBitrateFor(audioCodec) ? " (default)" : ""}
              </option>
            ))}
          </select>
        </label>
      ...
      <section className="space-y-4">
        {converting && (
          <ProgressBar
            value={progress}
            label={`Converting… (${Math.round(progress * 100)}%)`}
          />
        )}
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            ...
            disabled={runnableCount === 0 || converting || !patternValid}
        ...
        <p className="text-xs text-zinc-500">
          Copies the video stream and re-encodes audio to{" "}
          {codecLabel(audioCodec)} ({bitrateKbps} kbps). Keeps the source
          container. Never overwrites originals.
        </p>
      </section>
    </div>
  );
