...
const STORAGE = {
  targetContainer: "sak.video-remux.targetContainer",
  recursive: "sak.video-remux.recursive",
  outputDir: "sak.video-remux.outputDir",
  dropIncompatible: "sak.video-remux.dropIncompatible",
  namePattern: "sak.video-remux.namePattern",
} as const;
...
function makeJobId(): string {
  return `remux-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
}

export default function VideoRemuxTool() {
  ...
  const [targetContainer, setTargetContainer] = useState<TargetContainer>(() => {
    ...
    return isTargetContainer(stored) ? stored : "mp4";
  });
  const [outputDir, setOutputDir] = useState<string | null>(null);
  const [namePattern, setNamePattern] = useState(() =>
    loadString(STORAGE.namePattern, DEFAULT_NAME_PATTERN),
  );
  const [dropIncompatible, setDropIncompatible] = useState(() =>
    loadBool(STORAGE.dropIncompatible, false),
  );
  ...
  const [selectedStreamIndices, setSelectedStreamIndices] = useState<
    number[]
  >([]);
  ...
  useEffect(() => {
    saveBool(STORAGE.dropIncompatible, dropIncompatible);
  }, [dropIncompatible]);
    ...
    async function probeAll() {
      if (paths.length === 0) {
        if (!cancelled) {
          ...
          setSelectedStreamIndices([]);
        }
        return;
      }

      const nextStreams = new Map<string, MediaStreamInfo[]>();
      ...
      if (paths.length === 1) {
        const streams = nextStreams.get(paths[0]) ?? [];
        setSelectedStreamIndices(streams.map((s) => s.index));
      } else {
        setSelectedStreamIndices([]);
      }
    }
  ...
  useEffect(() => {
    ...
    const streams = streamsByPath.get(paths[0]) ?? [];
    setSelectedStreamIndices((prev) => {
      const valid = prev.filter((index) =>
        streams.some((s) => s.index === index),
      );
      if (valid.length > 0) return valid;
      return streams.map((s) => s.index);
    });
  }, [paths, streamsByPath, targetContainer]);
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      const draft = buildPreview(
        ...
        streamsByPath,
        dropIncompatible,
        paths.length === 1 ? selectedStreamIndices : null,
      );
  ...
  useEffect(() => {
    ...
    listen<FfmpegProgressPayload>("ffmpeg-progress", (event) => {
      ...
      if (total <= 0) return;
      const combined =
        (fileIndexRef.current + fileRatio) / total;
  ...
  function toggleStream(index: number) {
    setSelectedStreamIndices((prev) => {
  ...
  async function remuxAll() {
    ...
    for (let i = 0; i < ready.length; i++) {
      ...
      results.set(row.from, { status: "remuxing" });
      setRowResults(new Map(results));

      const streams = streamsByPath.get(row.from) ?? [];
      let indices = streamIndicesForRemux(
        streams,
        targetContainer,
        dropIncompatible,
        paths.length === 1 ? selectedStreamIndices : null,
      ...
      try {
        const result = await invoke<RemuxResult>("remux_video", {
          jobId,
          sourcePath: row.from,
          outputPath: row.to,
  ...
  const singleStreams = singleFile ? (streamsByPath.get(paths[0]) ?? []) : [];
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="block">
          ...
          <select
            ...
            className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm focus:border-brand/60 focus:outline-none"
          >
            {TARGET_CONTAINERS.map((container) => (
        ...
        <label className="flex items-center gap-2 text-sm text-zinc-400 sm:col-span-2">
          <input
            type="checkbox"
            checked={dropIncompatible}
            onChange={(e) => setDropIncompatible(e.target.checked)}
            className="rounded border-zinc-600 bg-zinc-900 text-brand focus:ring-brand/50"
          />
          Drop incompatible streams (batch remux maps only compatible streams)
        </label>
      </section>

      {singleFile && singleStreams.length > 0 && (
        <section className="space-y-2">
          <h2 className="text-sm font-medium text-zinc-300">Streams</h2>
          <ul className="rounded-xl border border-zinc-800 divide-y divide-zinc-800">
            {singleStreams.map((stream) => (
              <li key={stream.index} className="px-3 py-2">
                <label className="flex items-center gap-2 text-sm text-zinc-300">
                  <input
                    type="checkbox"
                    checked={selectedStreamIndices.includes(stream.index)}
                    onChange={() => toggleStream(stream.index)}
                    className="rounded border-zinc-600 bg-zinc-900 text-brand focus:ring-brand/50"
                  />
                  <span className="font-mono text-xs">{streamLabel(stream)}</span>
                </label>
              </li>
            ))}
          </ul>
          <p className="text-xs text-zinc-600">
            Uncheck streams to omit from the remux. Batch mode uses all streams
            or compatible-only when dropping incompatible.
          </p>
        </section>
      )}
      ...
      <section className="space-y-4">
        {remuxing && (
          <ProgressBar
            value={progress}
            label={`Remuxing… (${Math.round(progress * 100)}%)`}
          />
        )}
        ...
        <div className="flex flex-wrap items-center gap-4">
          <button
            type="button"
            onClick={() => void remuxAll()}
            disabled={runnableCount === 0 || remuxing || !patternValid}
          ...
          >
            {remuxing ? "Remuxing…" : `Remux (${runnableCount})`}
          </button>

          {remuxing && (
            <button
              type="button"
              onClick={() => void cancelRemux()}
          ...
          {summary && (
            <div className="flex flex-wrap items-center gap-3">
              <p className="text-sm text-zinc-300" role="status">{summary}</p>
              {lastSavedPath && (
                <button
                  type="button"
        ...
        <p className="text-xs text-zinc-500">
          Lossless stream-copy — codecs are preserved when the target container
          supports them. Incompatible streams are flagged; enable drop or pick
          streams for a single file.
        </p>
      </section>
    </div>
  );
